...
import { Lock, Megaphone, CheckCircle, List, Scissors, X, Check, Loader2, User, Clock } from 'lucide-vue-next'
...
const login = async () => {
  ...
  try {
    ...
    // Validar si la respuesta tiene token
    if (response.token) {
      ...
      if (userData.rol === 'Barbero') {
        ...
        error.value = "Acceso solo para barberos"
      }
    } else {
...
const cargarDatos = async () => {
  ...
  try {
    ...
    // Filtrar cola
    colaEnEspera.value = data.filter(t => t.estado === 'EnCola')
    // Buscar si estoy atendiendo a alguien
    turnoActual.value = data.find(t => 
...
const llamarSiguiente = async () => {
  ...
  try {
    ...
    showMessage("No se pudo llamar al siguiente (¿Quizás la cola está vacía?)", 'error')
  } finally {
  ...
  <div class="max-w-4xl mx-auto">
    ...
    <div v-if="!usuario" class="max-w-md mx-auto glass rounded-2xl p-8 border border-slate-800 shadow-xl mt-10">
      <div class="text-center mb-8">
        <div class="inline-flex items-center justify-center p-3 bg-red-600/10 rounded-xl mb-4 text-red-500">
          <Lock class="w-8 h-8" />
        </div>
        <h2 class="text-2xl font-bold text-white">Acceso Barbero</h2>
        <p class="text-slate-400">Ingresa tu PIN para comenzar</p>
      </div>
    ...
    <div v-else class="space-y-6">
      ...
      <div class="flex items-center justify-between mb-6">
        ...
        <div class="flex items-center gap-4">
          <!-- Connection Status -->
          <div class="flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold border transition-colors"
            :class="connectionState === 'Conectado' 
              ? 'bg-green-500/10 text-green-500 border-green-500/20' 
              : 'bg-red-500/10 text-red-500 border-red-500/20 animate-pulse'"
          >
            <div class="w-2 h-2 rounded-full" :class="connectionState === 'Conectado' ? 'bg-green-500' : 'bg-red-500'"></div>
            {{ connectionState }}
          </div>

          <button @click="logout" class="text-sm text-slate-400 hover:text-white underline decoration-slate-700 hover:decoration-white transition-all">
            Salir
          </button>
        </div>
      </div>
      ...
      <!-- Turno Actual Card -->
      <div v-if="turnoActual" class="glass rounded-2xl p-6 border border-slate-700 shadow-lg relative overflow-hidden group">
        <div class="absolute top-0 right-0 p-24 bg-red-600/5 group-hover:bg-red-600/10 transition-colors blur-3xl rounded-full pointer-events-none"></div>
        
        <div class="relative z-10">
          <div class="text-xs uppercase font-bold text-red-500 tracking-wider mb-2">
            {{ turnoActual.estado === 'Llamado' ? 'LLAMADO - ESPERANDO CLIENTE' : 'ATENDIENDO AHORA' }}
          </div>
          
          <div class="flex items-center justify-between mb-8">
            <div class="flex items-center gap-4">
              <div class="text-5xl font-black text-white">#{{ turnoActual.turnoDiario }}</div>
              <div>
                <div class="text-2xl font-bold text-white">{{ turnoActual.clienteNombre }}</div>
                <div class="text-slate-400 flex items-center gap-1.5 text-sm">
                   <Phone class="w-3.5 h-3.5" /> {{ turnoActual.clienteTelefono }}
                </div>
              </div>
            </div>
            
            <div v-if="turnoActual.estado === 'Llamado'" class="animate-pulse">
               <span class="px-3 py-1 bg-yellow-500/20 text-yellow-400 text-xs font-bold rounded-full border border-yellow-500/20">Esperando...</span>
            </div>
          </div>
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <template v-else>
              <button disabled class="bg-slate-800 text-green-500 font-bold py-4 rounded-xl border border-green-500/30 flex items-center justify-center gap-2 cursor-default">
      ...
      <div v-else class="glass rounded-2xl p-8 border border-slate-800 text-center py-16">
        ...
        <p class="text-slate-400 mb-8">La silla está vacía. Llama al siguiente cliente.</p>
      ...
      <div class="mt-8">
        ...
        <div class="space-y-3">
          <div v-for="turno in colaEnEspera" :key="turno.id" class="glass rounded-xl p-4 flex items-center justify-between border border-slate-800 hover:border-slate-700 transition-colors">
            <div class="flex items-center gap-4">
              <span class="text-xl font-black text-slate-500 w-8">#{{ turno.turnoDiario }}</span>
              <div>
                <div class="font-bold text-slate-200">{{ turno.clienteNombre }}</div>
                <div class="text-xs text-slate-500">{{ turno.clienteTelefono }}</div>
              </div>
            </div>
            <div class="px-3 py-1 bg-slate-900 rounded-full text-xs text-slate-400 border border-slate-800">
               En cola
            </div>
          </div>
          
          <div v-if="colaEnEspera.length === 0" class="text-center py-8 text-slate-500 border-2 border-dashed border-slate-800 rounded-xl">
    ...
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      <div class="glass w-full max-w-lg rounded-2xl p-6 border border-slate-700 shadow-2xl animate-in zoom-in-95 duration-200">
        <div class="flex items-center justify-between mb-6">
        ...
        <div class="space-y-2 mb-8 max-h-[60vh] overflow-y-auto pr-2">
          <div 
            ...
            :class="serviciosSeleccionados.includes(servicio.id) ? 'bg-red-600/10 border-red-500/50' : 'bg-slate-900/50 border-slate-800 hover:border-slate-700'"
          >
            <div class="flex items-center gap-3">
              <div class="w-5 h-5 rounded-full border flex items-center justify-center transition-colors"
                :class="serviciosSeleccionados.includes(servicio.id) ? 'bg-red-500 border-red-500' : 'border-slate-600'"
              >
                <Check v-if="serviciosSeleccionados.includes(servicio.id)" class="w-3 h-3 text-white" />
              </div>
              <span class="font-medium text-slate-200">{{ servicio.nombre }}</span>
            </div>
    ...
    <!-- Feedback Modal -->
    <div v-if="showFeedback" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      <div class="glass w-full max-w-sm rounded-2xl p-6 border border-slate-700 shadow-2xl animate-in zoom-in-95 duration-200 text-center">
        <div class="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4"
          :class="feedbackType === 'error' ? 'bg-red-500/10 text-red-500' : 'bg-green-500/10 text-green-500'"
        >
          <component :is="feedbackType === 'error' ? X : Check" class="w-8 h-8" />
        </div>
        ...
        <button 
          ...
          :class="feedbackType === 'error' ? 'bg-red-600 hover:bg-red-700 text-white' : 'bg-green-600 hover:bg-green-700 text-white'"
        >
          Entendido
        </button>
      </div>
