...
## 2026-08-10 - Insecure Direct Object Reference in State Transitions
**Vulnerability:** The `EnSilla` and `Finalizar` endpoints in `TurnosController` allowed any authenticated barber to modify the state of any turn, even those assigned to other barbers, because they only verified the `[Authorize]` attribute and the existence of the turn, lacking resource ownership validation.
**Learning:** Relying solely on role-based access control (`[Authorize]`) is insufficient for endpoints that mutate specific resources. When an action should only be performed by the resource owner (e.g., the barber assigned to a turn), the backend must explicitly verify ownership against the authenticated user's identity to prevent IDOR.
**Prevention:** Consistently extract the authenticated user's ID from claims (`NameIdentifier`) and validate it against the resource's owner identifier before allowing state changes, providing an exception only for elevated roles like Admin.
