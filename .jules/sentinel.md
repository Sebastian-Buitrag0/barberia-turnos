...
**Vulnerability:** The CORS configuration in `Program.cs` included `.SetIsOriginAllowed(origin => true)` alongside `.AllowCredentials()`.
**Learning:** This combination allows any origin to send authenticated cross-origin requests, bypassing CORS restrictions and potentially leaking sensitive data across origins.
**Prevention:** Always restrict CORS explicitly using `.WithOrigins()` to known and trusted origins, particularly when allowing credentials. Never blindly allow wildcard origins with credentials.
