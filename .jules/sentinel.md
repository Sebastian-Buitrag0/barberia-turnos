...
## 2025-02-14 - IDOR in Appointment State Modification
**Vulnerability:** The `EnSilla` and `Finalizar` endpoints in `TurnosController` permitted any authenticated user (barber) to change the state of any appointment, creating an Insecure Direct Object Reference (IDOR).
**Learning:** When endpoints act on specific entities modified by authenticated actors, role-based authorization alone (e.g., `[Authorize]`) is insufficient. Resource ownership validation is required to ensure actors only interact with resources assigned to them.
**Prevention:** Extract the user identifier from the claims principal (`User.FindFirst(ClaimTypes.NameIdentifier)`) and verify it matches the resource owner's ID (`Turno.BarberoId`), unless an overarching role (like "Admin") overrides the requirement.
